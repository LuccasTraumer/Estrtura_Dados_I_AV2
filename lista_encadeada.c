...
void inserir(TNo **cabeca)
{
    TNo *noatual, *novono;
    int v;
    printf("\nDigite um n�mero para armazenar na lista: ");
    scanf("%d", &v);
...
{
    char opcao;
    printf("1 -> Inserir um novo n�mero inteiro na lista\n");
    printf("2 -> Listar todos os n�meros inteiros da lista\n");
...
void remover(TNo** cabeca) {
    ...
    printf("\nDigite a posição do n�mero para remover na lista: ");
    scanf("%d", &posicaoRemover);

    if (temp == NULL) {
        printf("Item não presente na Lista!");
        return;
    }
    
    if (temp != NULL && posicaoRemover == 1) {
        (*cabeca) = temp->proximo;
        free(temp);
        return;
    }
    ...
    if(posicaoRemover > tamanhoLista) {
        printf("Posição invalida para remoção!");
        return;
    }
    
    int posicaoAtualLista = 1;
    TNo *temp = *cabeca, *anterior;
    
    while(temp != NULL && posicaoRemover != posicaoAtualLista) {
        anterior = temp;
        temp = temp->proximo;
        posicaoAtualLista = posicaoAtualLista +1;
    }
